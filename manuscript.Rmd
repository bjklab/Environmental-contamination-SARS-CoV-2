...
* __Results__: The probability of detecting SARS-CoV-2 RNA did not vary with distance. However, we found that surface type predicted probability of detection, with floors and high-touch surfaces having the highest probability of detection (floors odds ratio (OR) `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Q97.5_exp) %>% signif(.,3)`); high-touch elevated OR `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Q97.5_exp) %>% signif(.,3)`)). Patients requiring high-flow oxygen, positive airway pressure, or mechanical ventilation respiratory were associated with increased surface contamination (OR `r m_combined_summary %>% filter(model_name == "COVID-19 Severity") %>% filter(grepl("SignificantOxygen",param)) %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "COVID-19 Severity") %>% filter(grepl("SignificantOxygen",param)) %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "COVID-19 Severity") %>% filter(grepl("SignificantOxygen",param)) %>% pull(Q97.5_exp) %>% signif(.,3)`)). The probability of surface elevated contamination decayed with prolonged hospitalization, but the probability of floor detection increased with duration of the local pandemic wave.  
    
* __Conclusions__: Distance from patient's bed did not predict SARS-CoV-2 RNA deposition in patient rooms, but surface type, severity of illness, and time predicted surface deposition.   
...
* __Definition of Exposures and Outcomes__: The primary outcome of interest, detection of SARS-CoV-2 by RT-qPCR assay, was determined by detection of any quantifiable RNA on <span style="background-color:cyan">any of three technical replicates</span>. The primary exposures of interest were (1) distance from the head of patient's bed, measured as above; (2) time from COVID-19 diagnosis and from the start of the local COVID-19 case wave; and (3) COVID-19 clinical severity. We categorized COVID-19 severity by the highest level of oxygen support required during the subjects' admission, wither no oxygen support required (mild disease), only nasal or facemask oxygen support required (moderate disease), significant oxygen support (i.e., high-flow nasal cannula, continuous or bilevel positive airway pressure, or mechanical ventilation) required (severe disease).  
...
In conclusion, SARS-CoV-2 RNA surface deposition did not vary with distance within patient rooms. However, surface type, severity of illness, and time predicted detection of surface contamination. Future studies should focus on how the observed surface contamination contributes to risk of infection among patients and HCP, and on how cleaning strategies can target surfaces found to be at high risk of contamination.  
