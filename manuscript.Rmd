...
* __Background__: SARS-CoV-2 is known to contaminate surfaces within healthcare settings, but the spatial and temporal extent of environmental contamination has not been precisely defined. We sought to define SARS-CoV-2 contamination of different surface types and how contamination changes over time.  
...
* __Results__: The probability of detecting SARS-CoV-2 RNA did not vary with distance. However, we found that surface type predicted probability of detection, with floors and high-touch surfaces having the highest probability of detection (floors: `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Q97.5_exp) %>% signif(.,3)`); high-touch elevated: OR `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Q97.5_exp) %>% signif(.,3)`)). Patients requiring high-flow, continuous or bilevel positive airway pressure, or mechanical ventilation respiratory were associated with increased surface contamination (OR `r m_combined_summary %>% filter(model_name == "COVID-19 Severity") %>% filter(grepl("SignificantOxygen",param)) %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "COVID-19 Severity") %>% filter(grepl("SignificantOxygen",param)) %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "COVID-19 Severity") %>% filter(grepl("SignificantOxygen",param)) %>% pull(Q97.5_exp) %>% signif(.,3)`)). The probability of surface contamination decayed with prolonged hospitalization, but the probability of floor detection increased with duration of the local pandemic wave.  
...
* __Causal Model__: We hypothesized that SARS-CoV-2 contamination would decay with physical distance from patient and with time from COVID-19 diagnosis. We also hypothesized that high-touch objects would be more frequently contaminated with SARS-CoV-2 RNA than low-touch objects, and that increased COVID-19 disease severity would be associated with greater environmental contamination.     
...
* __SARS-CoV-2 RNA Environmental Contamination Varies with Surface Type But Not Distance from Patient__: We evaluated the relationship between the distance from the patient's head of the bed and the probability of detecting SARS-CoV-2 on environmental surfaces. There was no significant relationship between distance and the probability of SARS-CoV-2 detection, in contrast to prior studies of multidrug-resistant bacterial organisms [@Kelly2021-vm], but the suface type was significantly associated with the probability of detection. Floor surfaces had an odds ratio of `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_site_categoryFloor") %>% pull(Q97.5_exp) %>% signif(.,3)`) of SARS-CoV-2 detection, relative to elevated surfaces. High-touch elevated surfaces had an odds ratio of `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Estimate_exp) %>% signif(.,3)` (95% CrI `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Q2.5_exp) %>% signif(.,3)` to `r m_combined_summary %>% filter(model_name == "Distance Fixed" & param == "b_high_touchTRUE") %>% pull(Q97.5_exp) %>% signif(.,3)`) of SARS-CoV-2 detection, relative to low-touch elevated surfaces. **Figure 1A** depicts the results of a multivariable model relating distance from patient, surface location, and whether the surface was high-touch to the probability of SARS-CoV-2 detection.         
