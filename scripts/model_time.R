...
dat %>%
  brms::get_prior(data = ., family = bernoulli,
                  scv2_detected ~ (1 + subject_covid_day | site_category)
  ) %>%
  gt::gt()
...
rstan::check_hmc_diagnostics(m_binom_scv2_time_mix_category$fit)
m_binom_scv2_time_mix_category %>% pp_check()
...
#' fitted
m_binom_scv2_time_mix_category$data %>%
  as_tibble() %>%
...
m_binom_scv2_time_mix_category_fitted %>%
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        strip.background = element_blank()) -> p_scv2_binomial_subject_covid_day_site_category
p_scv2_binomial_subject_covid_day_site_category


# p_scv2_binomial_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_binomial_subject_covid_day_site_category.pdf", height = 5, width = 8, units = "in")
# p_scv2_binomial_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_binomial_subject_covid_day_site_category.png", height = 5, width = 8, units = "in", dpi = 600)
# p_scv2_binomial_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_binomial_subject_covid_day_site_category.svg", height = 5, width = 8, units = "in")
...
rstan::check_hmc_diagnostics(m_binom_scv2_time_mix_category_touch$fit)
m_binom_scv2_time_mix_category_touch %>% pp_check()
...
#' fitted
m_binom_scv2_time_mix_category_touch$data %>%
  as_tibble() %>%
  expand(subject_covid_day = modelr::seq_range(subject_covid_day, n = 100),
         site_category = unique(site_category),
         high_touch = unique(high_touch)
...
m_binom_scv2_time_mix_category_touch_fitted %>%
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        legend.position = "top",
        #legend.background = element_rect(fill = "white", color = "black", size = 0.25),
        strip.background = element_blank()) -> p_scv2_binomial_subject_covid_day_site_category_touch
p_scv2_binomial_subject_covid_day_site_category_touch


# p_scv2_binomial_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_binomial_subject_covid_day_site_category_touch.pdf", height = 5, width = 8, units = "in")
# p_scv2_binomial_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_binomial_subject_covid_day_site_category_touch.png", height = 5, width = 8, units = "in", dpi = 600)
# p_scv2_binomial_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_binomial_subject_covid_day_site_category_touch.svg", height = 5, width = 8, units = "in")
...
dat %>%
  brms::get_prior(data = ., family = bernoulli,
                  log10(copies_max) ~ (1 + subject_covid_day | site_category)
  ) %>%
  gt::gt()
...
rstan::check_hmc_diagnostics(m_linear_scv2_time_mix_category$fit)
m_linear_scv2_time_mix_category %>% pp_check()
...
#' fitted
m_linear_scv2_time_mix_category$data %>%
  as_tibble() %>%
  expand(subject_covid_day = modelr::seq_range(subject_covid_day, n = 100),
         site_category = unique(site_category)#,
         #high_touch = unique(high_touch)
...
m_linear_scv2_time_mix_category_fitted %>%
  ...
  geom_point(data = mutate(as_tibble(m_linear_scv2_time_mix_category$data),
             ...
             aes(x = subject_covid_day, y = log10(copies_max)), color = "grey", alpha = 0.5) +
  #facet_wrap(facets = ~ site_category + high_touch, scales = "fixed") +
  facet_wrap(facets = ~ site_category) +
  scale_fill_brewer(palette = "Reds") +
  labs(x = "Days after COVID-19 Diagnosis",
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        legend.position = "top",
        #legend.background = element_rect(fill = "white", color = "black", size = 0.25),
        strip.background = element_blank()) -> p_scv2_linear_subject_covid_day_site_category
p_scv2_linear_subject_covid_day_site_category


# p_scv2_linear_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_linear_subject_covid_day_site_category.pdf", height = 5, width = 8, units = "in")
# p_scv2_linear_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_linear_subject_covid_day_site_category.png", height = 5, width = 8, units = "in", dpi = 600)
# p_scv2_linear_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_linear_subject_covid_day_site_category.svg", height = 5, width = 8, units = "in")
...
dat %>%
  brms::get_prior(data = ., family = bernoulli,
                  log10(copies_max) ~ (1 + subject_covid_day | site_category / high_touch)
  ) %>%
  gt::gt()
...
rstan::check_hmc_diagnostics(m_linear_scv2_time_mix_category_touch$fit)
m_linear_scv2_time_mix_category_touch %>% pp_check()
...
#' fitted
m_linear_scv2_time_mix_category_touch$data %>%
  as_tibble() %>%
  expand(subject_covid_day = modelr::seq_range(subject_covid_day, n = 100),
         site_category = unique(site_category),
         high_touch = unique(high_touch)
...
m_linear_scv2_time_mix_category_touch_fitted %>%
  ...
  geom_point(data = mutate(as_tibble(m_linear_scv2_time_mix_category_touch$data),
                           subject_covid_day = subject_covid_day * sd(dat$subject_covid_day, na.rm = TRUE) + mean(dat$subject_covid_day, na.rm = TRUE),
                           high_touch = case_when(high_touch == FALSE ~ "Low Touch",
                                                  high_touch == TRUE ~ "High Touch")),
             aes(x = subject_covid_day, y = log10(copies_max)), color = "grey", alpha = 0.5) +
  facet_wrap(facets = ~ site_category + high_touch, scales = "fixed") +
  #facet_wrap(facets = ~ site_category) +
  scale_fill_brewer(palette = "Reds") +
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        legend.position = "top",
        #legend.background = element_rect(fill = "white", color = "black", size = 0.25),
        strip.background = element_blank()) -> p_scv2_linear_subject_covid_day_site_category_touch
p_scv2_linear_subject_covid_day_site_category_touch


# p_scv2_linear_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_linear_subject_covid_day_site_category_touch.pdf", height = 5, width = 8, units = "in")
# p_scv2_linear_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_linear_subject_covid_day_site_category_touch.png", height = 5, width = 8, units = "in", dpi = 600)
# p_scv2_linear_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_linear_subject_covid_day_site_category_touch.svg", height = 5, width = 8, units = "in")
...
dat %>%
  ...
  select(copies_max, subject_covid_day, site_category) %>%
  mutate(subject_covid_day = scale(subject_covid_day)[,1]) %>%
  brms::get_prior(data = ., formula = copies_max ~ 1 + (1 + subject_covid_day | site_category),
...
rstan::check_hmc_diagnostics(m_hurdle_scv2_time_mix_category$fit)
m_hurdle_scv2_time_mix_category %>% pp_check() + scale_x_log10()
...
#' fitted
m_hurdle_scv2_time_mix_category$data %>%
  as_tibble() %>%
  expand(subject_covid_day = modelr::seq_range(subject_covid_day, n = 100),
         site_category = unique(site_category)#,
         #high_touch = unique(high_touch)
...
m_hurdle_scv2_time_mix_category_fitted %>%
  ...
  geom_point(data = mutate(as_tibble(m_hurdle_scv2_time_mix_category$data),
             ...
             aes(x = subject_covid_day, y = copies_max), color = "grey", alpha = 0.5) +
  #facet_wrap(facets = ~ site_category + high_touch, scales = "fixed") +
  facet_wrap(facets = ~ site_category) +
  scale_y_log10() +
  scale_fill_brewer(palette = "Reds") +
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        legend.position = "top",
        #legend.background = element_rect(fill = "white", color = "black", size = 0.25),
        strip.background = element_blank()) -> p_scv2_hurdle_subject_covid_day_site_category
p_scv2_hurdle_subject_covid_day_site_category


# p_scv2_hurdle_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_hurdle_subject_covid_day_site_category.pdf", height = 5, width = 8, units = "in")
# p_scv2_hurdle_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_hurdle_subject_covid_day_site_category.png", height = 5, width = 8, units = "in", dpi = 600)
# p_scv2_hurdle_subject_covid_day_site_category %>%
#   ggsave(filename = "./figs/p_scv2_hurdle_subject_covid_day_site_category.svg", height = 5, width = 8, units = "in")
...
#' ####################################################
#' generative model for SARS-CoV-2 contamination ~ days from COVID-19 diagnosis
#' - elevated vs floor -- accounting for high-touch
#' - hurdle quantity detected
#' ####################################################
...
dat %>%
  ...
  select(copies_max, subject_covid_day, site_category, high_touch) %>%
  mutate(subject_covid_day = scale(subject_covid_day)[,1]) %>%
  brms::get_prior(data = ., formula = copies_max ~ 1 + (1 + subject_covid_day | site_category / high_touch),
                  family = hurdle_lognormal()) %>%
  gt::gt()
...
rstan::check_hmc_diagnostics(m_hurdle_scv2_time_mix_category_touch$fit)
m_hurdle_scv2_time_mix_category_touch %>% pp_check() + scale_x_log10()
...
#' fitted
m_hurdle_scv2_time_mix_category_touch$data %>%
  as_tibble() %>%
  expand(subject_covid_day = modelr::seq_range(subject_covid_day, n = 100),
         site_category = unique(site_category),
         high_touch = unique(high_touch)
...
m_hurdle_scv2_time_mix_category_touch_fitted %>%
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        legend.position = "top",
        #legend.background = element_rect(fill = "white", color = "black", size = 0.25),
        strip.background = element_blank()) -> p_scv2_hurdle_subject_covid_day_site_category_touch
p_scv2_hurdle_subject_covid_day_site_category_touch


# p_scv2_hurdle_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_hurdle_subject_covid_day_site_category_touch.pdf", height = 5, width = 8, units = "in")
# p_scv2_hurdle_subject_covid_day_site_category_touch %>%
...
#' model comparison
#' ######################
loo::loo_compare(loo(m_mvbinom_scv2_time_fix_category_touch), loo(m_binom_scv2_time_mix_category_touch), loo(m_binom_scv2_time_mix_category)) %>%
  identity() -> binom_time_category_touch_loo_comparison
binom_time_category_touch_loo_comparison
...
dat %>%
  ...
  select(copies_max, subject_covid_day, site_category, high_touch) %>%
  mutate(subject_covid_day = scale(subject_covid_day)[,1]) %>%
  brms::get_prior(data = ., formula = copies_max ~ 1 + subject_covid_day + site_category + high_touch,
                  family = hurdle_lognormal()) %>%
  gt::gt()
...
#   select(copies_max, subject_covid_day, site_category, high_touch) %>%
#   mutate(subject_covid_day = scale(subject_covid_day)[,1]) %>%
#   brm(formula = copies_max ~ 1 + subject_covid_day + site_category + high_touch,
#       data = .,
#       family = hurdle_lognormal(),
#       chains = 4,
#       cores = 4,
#       control = list("adapt_delta" = 0.999, max_treedepth = 10),
#       backend = "cmdstanr",
#       seed = 16) -> m_mvhurdle_scv2_time_fix_category_touch
# 
# m_mvhurdle_scv2_time_fix_category_touch %>% write_rds(file = "./models/hurdle/m_mvhurdle_scv2_time_fix_category_touch.rds.gz", compress = "gz")
m_mvhurdle_scv2_time_fix_category_touch <- read_rds(file = "./models/hurdle/m_mvhurdle_scv2_time_fix_category_touch.rds.gz")

m_mvhurdle_scv2_time_fix_category_touch
rstan::check_hmc_diagnostics(m_mvhurdle_scv2_time_fix_category_touch$fit)
m_mvhurdle_scv2_time_fix_category_touch %>% pp_check() + scale_x_log10()
...
#' fitted
m_mvhurdle_scv2_time_fix_category_touch$data %>%
  as_tibble() %>%
  expand(subject_covid_day = modelr::seq_range(subject_covid_day, n = 100),
         site_category = unique(site_category),
         high_touch = unique(high_touch)
  ...
  filter(!(high_touch == TRUE & site_category == "Floor")) %>%
  add_epred_draws(m_mvhurdle_scv2_time_fix_category_touch) %>%
  mutate(subject_covid_day = subject_covid_day * sd(dat$subject_covid_day, na.rm = TRUE) + mean(dat$subject_covid_day, na.rm = TRUE)) %>%
  identity() -> m_mvhurdle_scv2_time_fix_category_touch_fitted
m_mvhurdle_scv2_time_fix_category_touch_fitted
...
m_mvhurdle_scv2_time_fix_category_touch_fitted %>%
  ...
  ggplot(aes(x = subject_covid_day, y = .epred)) +
  stat_lineribbon() +
  geom_point(data = mutate(as_tibble(m_mvhurdle_scv2_time_fix_category_touch$data),
                           subject_covid_day = subject_covid_day * sd(dat$subject_covid_day, na.rm = TRUE) + mean(dat$subject_covid_day, na.rm = TRUE),
                           high_touch = case_when(high_touch == FALSE ~ "Low Touch",
                                                  high_touch == TRUE ~ "High Touch")),
             aes(x = subject_covid_day, y = copies_max), color = "grey", alpha = 0.5) +
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        legend.position = "top",
        #legend.background = element_rect(fill = "white", color = "black", size = 0.25),
        strip.background = element_blank()) -> p_scv2_mvhurdle_subject_covid_day_site_category_touch
p_scv2_mvhurdle_subject_covid_day_site_category_touch


# p_scv2_mvhurdle_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_mvhurdle_subject_covid_day_site_category_touch.pdf", height = 5, width = 8, units = "in")
# p_scv2_mvhurdle_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_mvhurdle_subject_covid_day_site_category_touch.png", height = 5, width = 8, units = "in", dpi = 600)
# p_scv2_mvhurdle_subject_covid_day_site_category_touch %>%
#   ggsave(filename = "./figs/p_scv2_mvhurdle_subject_covid_day_site_category_touch.svg", height = 5, width = 8, units = "in")


#' ######################
#' model comparison
#' ######################
loo::loo_compare(loo(m_mvhurdle_scv2_time_fix_category_touch), loo(m_hurdle_scv2_time_mix_category_touch), loo(m_hurdle_scv2_time_mix_category)) %>%
  identity() -> hurdle_time_category_touch_loo_comparison
hurdle_time_category_touch_loo_comparison
...
m_mvbinom_scv2_time_category_touch_mix_subject_fitted %>%
  ...
  theme(strip.text = ggtext::element_markdown(color = "black", size = 8),
        ...
        legend.position = "top",
        #legend.background = element_rect(fill = "white", color = "black", size = 0.25),
        strip.background = element_blank()) -> p_scv2_mvbinomial_time_site_category_touch_mix_subject
p_scv2_mvbinomial_time_site_category_touch_mix_subject


# p_scv2_mvbinomial_time_site_category_touch_mix_subject %>%
#   ggsave(filename = "./figs/p_scv2_mvbinomial_time_site_category_touch_mix_subject.pdf", height = 5, width = 8, units = "in")
# p_scv2_mvbinomial_time_site_category_touch_mix_subject %>%
#   ggsave(filename = "./figs/p_scv2_mvbinomial_time_site_category_touch_mix_subject.png", height = 5, width = 8, units = "in", dpi = 600)
# p_scv2_mvbinomial_time_site_category_touch_mix_subject %>%
#   ggsave(filename = "./figs/p_scv2_mvbinomial_time_site_category_touch_mix_subject.svg", height = 5, width = 8, units = "in")
...
((p_scv2_mvbinomial_subject_covid_day_site_category_touch + labs(y = "Probability of SARS-CoV-2<br>Detection by RT-PCR")) / 
    (p_scv2_mvbinomial_time_site_category_touch_mix_subject_lines + labs(y = "Probability of SARS-CoV-2<br>Detection by RT-PCR"))) +
  patchwork::plot_annotation(tag_levels = "A") -> p_time_models_combined
p_time_models_combined


# p_time_models_combined %>%
#   ggsave(filename = "./figs/p_time_models_combined.pdf", height = 7, width = 8, units = "in")
# p_time_models_combined %>%
#   ggsave(filename = "./figs/p_time_models_combined.png", height = 7, width = 8, units = "in", dpi = 600)
# p_time_models_combined %>%
#   ggsave(filename = "./figs/p_time_models_combined.svg", height = 7, width = 8, units = "in")


#' examine relationship between subjects' floors and high-touch elevated
m_mvbinom_scv2_time_category_touch_mix_subject_fitted %>%
  filter(subject_covid_day == 0) %>%
...
#' ############################
#' TIME MODELS WITH DAYS POST ROOM OCCUPENCY (in addition to days post COVID-19)
#' - evaluate the effect of changing rooms
#' ############################
